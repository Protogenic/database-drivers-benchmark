...
func sqliteUtil(query int) {
	db, _ := sql.Open("sqlite3", "identifier.sqlite")
	var rows *sql.Rows
	var err error
	switch query {
	case 1:
	...
	case 3:
		for i := 0; i < 10; i++ {
			rows, err = db.Query("SELECT passenger_count, strftime('%Y', pickup_datetime), count(*) FROM trips GROUP BY 1, 2")
	...
	case 4:
		for i := 0; i < 10; i++ {
			rows, err = db.Query("SELECT passenger_count, strftime('%Y', pickup_datetime),  round(trip_distance), count(*) FROM trips GROUP BY 1, 2, 3 ORDER BY 2, 4 desc")
